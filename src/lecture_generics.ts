...
function genericLogic( x:any, timeout:number):any{
...
genericLogic("10",500).then((e)=>{console.log(e)});

// 우의 코드에서 then에서 나오는 return 값을 보면 any로 나옴.

function genericLogic2<T>( x:T, timeout:number){
    // return new Promise(( resolve ,reject)=>{
    //     setTimeout(() => {
    //         resolve(x);
    //     }, timeout);
...
genericLogic2(30,500).then((e)=>{console.log(e)});

 
genericLogic2<string>("30",500).then((e)=>{console.log(e)});
//이렇게 제너릭이 들어간 함수를 호출 할 때 함수의 타입을 제너릭처럼 보내주어도 됨.

function genericLogic3<T,D,U>(v:T,v2:D,v3:U):[T,D,U]{
    return [v,v2,v3];
}

const createTuple = genericLogic3("dd",3,1);
